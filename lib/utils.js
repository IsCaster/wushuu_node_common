...
function setRangeParams(rangeKey, condition) {
    for (var i of rangeKey) {
        var key = i.key
        var value = condition[i.key]
        if (value != undefined && (value.gt != undefined || value.lt != undefined || value.gte != undefined || value.lte != undefined)) {
            let queryArr = []
            ...
            if (value.gte != undefined) {
                queryArr.push({
                    [key]: orm.gte(value.gte)
                })
            }
            if (value.lte != undefined) {
                queryArr.push({
                    [key]: orm.lte(value.lte)
                })
            }
            delete condition[key]
            condition.and = queryArr
        }
    }
    return condition
}
...
function objectEquals(x, y) {
    ...
    if (x instanceof Function) {
        //console.log("x=", x.toString().replace(/ +/g, " "))
        //console.log("y=", y.toString().replace(/ +/g, " "))
        return x.toString().replace(/ +/g, " ") === y.toString().replace(/ +/g, " ")
    }
