...
var AnsyMap = require('./db').ansyMap
var defineTable = require('./orm/pgsql.db.config').defineTable;
...
function Model(modelName, conf) {
    ...
    if (conf.PG) {
        return defineTable(modelName, conf)
    } else
...
Model_actor.prototype.del = function(index, _callback) {
    ...
    var action = function() {
        ...
        var callback = _callback || function() {};
        var dao = $this.createDao();
        var ansyMap = new AnsyMap(_selected);
...
Model_actor_SET.prototype.save = function(_callback) {
    ...
    var action = function() {
        var callback = _callback || function() {};
        var dao = $this.createDao();
        var ansyMap = new AnsyMap($this.properties);
        ansyMap.onDone = function() {
            var tag = callback(this.results);
            if (tag == undefined || tag == true) {
                // $this.is_tail_called = true;
...
function loadConf(modelConf) {
    return Promise.all(Object.keys(modelConf).map(modelName => Model(modelName, modelConf[modelName])))
}

function init(modelConf) {
    var DB = require('./db').DB
    var sync = require('./orm/pgsql.db.config').sync;
    if (!(g_db instanceof DB))
        g_db = new DB()

    return dao_module.init()
        .then(() => loadConf(modelConf))
        .then(() => sync())
        .then(() => g_db.q_connect())
}
