...
var add_gen = function(MODEL_NAME, MODEL_NAME_URL, before, necessary_attr, firstHandler) {
    return function(ctx) {
        var handler = function(req, res) {
            res.header('Content-Type', 'application/json');
            co(function*() {
        ...
        if (firstHandler instanceof Function) {
            handlerArr = [firstHandler, handler]
        } else {
            handlerArr = [handler]
        }

        ctx.express.post('/' + MODEL_NAME_URL + '/add', handlerArr);
    }
}
