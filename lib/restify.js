    ...
    return function(ctx) {
        var handler = function(req, res) {
            ...
            co(function*() {
                ...
                var attr = Model_factory(MODEL_NAME).model.attr
                var all_keys = Model_factory(MODEL_NAME).keys
                checkValid(req, all_keys)
                var condition = utils.getUsefulKeys(all_keys, req, 'GET')
                var rangeKey = []
                attr.map(obj => {
                    if (obj.rangeQuery == true)
                        rangeKey.push(obj.key)
                })
                condition = utils.setRangeParams(rangeKey, condition)
                if (req.query.id != undefined) {
                    condition.id = req.query.id
                }

                if (req.query.or != undefined) {
                    condition.or = req.query.or
                }
                ...
                else if (page_size == undefined && limit != undefined)
                    page_size = parseInt(limit, 10)
                else
                    page_size = 1000

                console.log('./web/common.db.op.js-list_gen():condition = ' + JSON.stringify(condition))
...
var save2pg = function(MODEL_NAME, req, res) {
    return co(function*() {
        ...
        var handler = function(req, res) {
            ...
            co(function*() {
                ...
                if (uniqueKey != undefined && valid != undefined) {
                    ...
                    if (entity.length != 0) {
                        entity[0].valid = 1
                        entity[0][UNIQUE_KEY] = entity[0][UNIQUE_KEY] + '_' + Math.floor(Math.random() * 10) + '_' + (new Date()).getTime()
                        yield new Promise(function(resolve, reject) {
                            entity[0].save(function(err, result) {
                                if (err) {
                                    reject(err)
                                } else {
                                    resolve()
                ...
                yield save2pg(MODEL_NAME, req, res)
            }).catch(function(err) {
                utils.failHandler(res, err.message)
