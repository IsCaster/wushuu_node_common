...
var add_gen = function(MODEL_NAME, MODEL_NAME_URL, before, necessary_attr, TABLE_DEFINE) {
    return function(ctx) {
        var handler = function(req, res) {
            res.header('Content-Type', 'application/json');
            co(function*() {
                ...
                if (TABLE_DEFINE != undefined) {
                    var modelJson = require(TABLE_DEFINE)
                    var obj = _.find(modelJson[MODEL_NAME].attr, function(item) {
                        return item.key == 'valid'
                    })
                    var obj_unique = _.find(modelJson[MODEL_NAME].attr, function(item) {
                        return item.unique = true
                    })
                }

                if (obj != undefined && obj_unique != undefined) {
                    var UNIQUE_KEY = obj.key
                    var where = {}
