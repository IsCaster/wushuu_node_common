        ...
        var handler = function(req, res) {
            ...
            co(function*() {
                ...
                if (req.query.time != undefined) {
                    ...
                    if (time instanceof Array || (typeof time) == "string") {
                        where.time = time
                    } else if (time != undefined && (time.gt != undefined || time.lt != undefined || time.gte != undefined || time.lte != undefined)) {
                        let queryArr = []
                        if (time.gt != undefined) {
                            queryArr.push({
                                time: orm.gt(time.gt)
                            })
                        }
                        if (time.lt != undefined) {
                            queryArr.push({
                                time: orm.lt(time.lt)
                            })
                        }
                        if (time.gte != undefined) {
                            queryArr.push({
                                time: orm.gte(time.gte)
                            })
                        }
                        if (time.lte != undefined) {
                            queryArr.push({
                                time: orm.lte(time.lte)
                            })
                        }
                        delete where.time
                        where.and = queryArr
                ...
                var data = yield new Promise(function(resolve, reject) {
                    ...
                    }, limit, order, function(err, result) {
                        if (err) {
                            reject(err)
                        } else {
                            resolve(result)
