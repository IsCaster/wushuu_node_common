...
function _init() {
    return co(function*() {
        ...
        g_db = yield q_orm_connect(config.pg_conn_str)
        q_db_sync = promisify(db.sync, {
            context: g_db
        })
    }).catch(function(err) {
...
function defineTable(name, conf) {

}

function registerTable(name, conf) {
    defineTable(name, conf)
    return q_db_sync()
}
...
function transformConf(conf) {
    var params = []
    params[0] = conf.tableName || conf.name
    params[1] = {}
    params[1].id = {
        type: 'serial',
        key: true
    ...
    conf.attr.map(a => {
        ...
        if (a.type === "int") {
            ...
            params[1][a.key].size = 8
        } else if (a.type === "string") {
            params[1][a.key].type = 'text'
        } else if (a.type === "date") {
            params[1][a.key].type = 'data'
