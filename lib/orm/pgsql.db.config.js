...
var define_tables = function(con) {
    ...
    var shop_member = con.define('shop_member', {
        ...
        create_time: { type: 'date', time: true },
        update_time: { type: 'date', time: true },
        recent_time: { type: 'date', time: true },
...
function initdb() {
    return co(function*() {
        var q_orm_connect = promisify(orm.connect, { context: orm })
        var db = yield q_orm_connect(config.pg_conn_str)
        var q_db_sync = promisify(db.sync, { context: db })
        var tables = define_tables(db)
        yield q_db_sync
        return tables
    }).catch(function(err) {
